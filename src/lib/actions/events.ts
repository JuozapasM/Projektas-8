...
export async function createTeamAction(eventId: string, formData: FormData) {
  const name = textField(formData, 'team_name'); const capacity = integerField(formData, 'capacity');
...
function eventFields(formData: FormData) {
  ...
  if (formData.has('description') && typeof formData.get('description') !== 'string') return null;
  const startsAt = textField(formData, 'starts_at');
...
export async function saveResultAction(eventId: string, formData: FormData) {
  const table = integerField(formData, 'table_number'); const points = integerField(formData, 'points'); const name = textField(formData, 'team_name');
