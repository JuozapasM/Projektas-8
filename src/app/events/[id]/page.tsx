...
import { isUuid, formatEventDate, eventStatusLabels, maxTeamCapacity } from '@/lib/events';
...
export default async function EventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  let registration: EventRegistration | null = null; let registrationError = false;
  if (user) { const { data, error } = await supabase.from('event_registrations').select('*, event_teams(*)').eq('event_id', id).eq('user_id', user.id).maybeSingle(); registration = data as EventRegistration | null; registrationError = !!error; }
  ...
  return <div className="max-w-6xl mx-auto px-5 sm:px-8"><Link href="/" className="text-sm text-slate-400 hover:text-amber-200">← Visi renginiai</Link><div className="grid lg:grid-cols-2 gap-8 items-start mt-6 mb-9"><header><p className="text-xs text-amber-200 mb-3">{eventStatusLabels[event.status]}</p><h1 className="text-3xl sm:text-4xl font-bold tracking-tight">{event.title}</h1><p className="mt-4 text-slate-300">{formatEventDate(event.starts_at)}</p><p className="mt-2 text-sm text-slate-400">{event.location} · Lietuvos laikas</p><p className="mt-5 text-sm text-slate-400 whitespace-pre-wrap leading-relaxed">{event.description}</p></header><EventControls event={event} userId={user?.id} registration={registration} freeSeats={seats.filter(s => !s.user_id && !s.team_id).length} origin={origin} queuePosition={queuePosition} maxTeamSize={maxTeamCapacity(seats)} teamFreeSlots={registration?.team_id ? seats.filter(s => s.team_id === registration.team_id && !s.user_id).length : 0} isAvailable={!registrationError && !seatsError && seats.length === 24} /></div>
