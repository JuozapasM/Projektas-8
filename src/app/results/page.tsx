...
export default async function ResultsPage() {
  ...
  if (getSupabaseEnv().isConfigured) {
    ...
    if (!error) { available = true; events = (data || []) as GameEvent[]; if (events.length) { const { data, error: resultsError } = await supabase.from('event_results').select('*').in('event_id',events.map(e => e.id)); if (resultsError) { available = false; events = []; } else results = (data || []) as EventResult[]; } }
  }
