...
export function EventControls({ event, userId, registration, freeSeats, origin, queuePosition }: { event: GameEvent; userId?: string; registration: EventRegistration | null; freeSeats: number; origin: string; queuePosition?: number | null }) {
  ...
  return <section className="panel">
    ...
    {!userId ? <><h2 className="text-xl font-semibold">Susitinkame prie stalo.</h2><p className="my-4 text-sm text-slate-400">Prisijunkite ir rezervuokite vietą sau arba savo komandai.</p><div className="flex flex-wrap gap-3"><Link className="primary-button" href={`/auth/login?next=/events/${event.id}`}>Prisijungti</Link><Link className="secondary-button" href={`/auth/register?next=/events/${event.id}`}>Registruotis</Link></div></> : active ? <>
      ...
      {registration.status === 'waiting' ? <p className="my-4 text-sm text-slate-400 flex gap-2"><Clock3 size={18} className="shrink-0" aria-hidden="true" />{open ? `Jūsų vieta eilėje: ${queuePosition || 'tikslinama'}. Atlaisvinta vieta bus priskirta automatiškai. Savo būseną rasite šiame puslapyje.` : 'Renginio registracija uždaryta. Laukiantiems žaidėjams vietos šiuo metu nebeskirstomos.'}</p> : <p className="my-4 text-sm text-slate-400">Jūsų bilietas ir atvykimo QR kodas pateikti žemiau.{team && ` Komanda: ${team.name}.`}</p>}
      {team?.owner_id === userId && open && <div className="my-5 border-t border-slate-700 pt-4"><p className="text-sm text-slate-300 mb-3">Pakvieskite draugus į likusias {team.capacity - 1} komandos vietas. Pakvietimas veikia tik kol yra komandai laikomų vietų.</p><CopyLink url={`${origin}/invite/${team.invite_token}`} /></div>}
